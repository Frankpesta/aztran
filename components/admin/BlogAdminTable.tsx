...
export function BlogAdminTable(): ReactElement {
  const staffReady = useConvexStaffSessionReady();
  const rows = useQuery(api.blogPosts.getAllBlogPosts, staffReady ? {} : "skip") ?? [];
  const verifyHuman = useRecaptchaGate();
  const remove = useMutation(api.blogPosts.deleteBlogPost);
            ...
            <TableCell className="space-x-2 text-right">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  void (async () => {
                    try {
                      await verifyHuman("admin_blog_delete");
                    } catch {
